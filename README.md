...
This feature allows Redmine to connect to an AWS RDS PostgreSQL database using credentials dynamically retrieved from AWS Secrets Manager. Follow the steps below to configure and use this functionality.

#### **Configuration**

Use `docker-compose-aws.yml` as a starting point and set the following environment variables under the `redmine` service:
...
2. Ensure the `DB_CREATE=false` environment variable is set to avoid Redmine overwriting the database.
