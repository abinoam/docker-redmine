...
Current Version: **sameersbn/redmine:6.0.0**
...
```bash
docker pull sameersbn/redmine:6.0.0
```
...
```bash
docker run --name redmine -it --rm [OPTIONS] \
  sameersbn/redmine:6.0.0 app:backup:create
```
...
```bash
docker run --name redmine -it --rm [OPTIONS] \
  sameersbn/redmine:6.0.0 app:backup:restore
```
...
```bash
docker run --name redmine -it --rm [OPTIONS] \
  sameersbn/redmine:6.0.0 app:backup:restore BACKUP=1417624827_redmine_backup.tar
```
...
docker run --name=redmine -d [OPTIONS] \
  sameersbn/redmine:6.0.0 app:rake redmine:email:test[admin]
```
...
```bash
docker run --name=redmine -d [OPTIONS] \
  sameersbn/redmine:6.0.0 app:rake redmine:attachments:prune
```
...
```bash
docker run --name=redmine -d [OPTIONS] sameersbn/redmine:6.0.0
```
...
- Commands to run to update image to next redmine release, examples are from 6.0.0 to 6.0.1

```bash
sed -i 's/6.0.0/6.0.1/g' VERSION README.md docker-compose-memcached.yml docker-compose-mysql.yml docker-compose-ssl.yml docker-compose-sqlite3.yml docker-compose-mariadb.yml Dockerfile docker-compose.yml
...
- Select tag 6.0.0 and set release title to 6.0.0
- Publish release
